...
# //////////////////////////////////////////////////////////////////////////////////////////////////
@app.post("/resetemail")
async def get_switches(email: str = Form(...)):
    # try:
        print("Email:", email)
        cursor = connection.cursor()
        cursor.execute("SELECT id, email, username, state FROM users WHERE id = %s", (Id,))
        email = cursor.fetchall()
        cursor.close()

        if not users:
            return JSONResponse(content={'error': "No User Exists"}, status_code=500)
        return JSONResponse(content={'Link sent': email}, status_code=200)
    except Exception as e:
        traceback_str = traceback.format_exc()
        print("Error: ", traceback_str)
        return JSONResponse(content={'error': str(e)}, status_code=500)
